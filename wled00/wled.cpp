...
void WLED::initConnection()
{
  ...
  if (multiWiFi.empty()) {                       // guard: handle empty WiFi list safely (from V5)
    WiFi.config(IPAddress((uint32_t)0), IPAddress((uint32_t)0), IPAddress((uint32_t)0));
  } else {
    if (selectedWiFi >= multiWiFi.size()) selectedWiFi = 0; // guard: ensure valid index (from V5)
// AI: below section was generated by an AI ...
    if (uint32_t(multiWiFi[selectedWiFi].staticIP) != 0U) {  // explicit cast to uint32_t ensures we check the IPv4 address, not IPv6
      ...
      // Always pass the configured gateway to WiFi.config(). The upstream
      // staticGW != 0 requirement is intentionally dropped: default route
      // selection between interfaces is handled by setPrimaryNetworkInterface().
      // Gateway of 0.0.0.0 is explicitly supported for users who want WiFi
      // as a stub interface with no onward routing.
