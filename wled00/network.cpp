...
// AI: IDF v4 path only — manual esp-netif iteration in setPrimaryNetworkInterface()
// requires these headers. On IDF v5 / arduino-esp32 3.x the NetworkInterface API
// (ETH.setDefault() / WiFi.STA.setDefault()) is used instead, no direct lwIP access needed.
...
#if defined(ESP_IDF_VERSION) && (ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0))
  // AI: below section was generated by an AI
  // IDF v5 / arduino-esp32 3.x path: the core's NetworkInterface class (which
  // both ETHClass and STAClass inherit) provides setDefault(), wrapping
  // esp_netif_set_default_netif() with correct locking. This replaces the
  // manual esp-netif iteration and deprecated esp_netif_next() used on V4.
